...
<div class="page-wrap">
  ...
  <div class="group-1">
    ...
    <!-- ═══ 기존 설비 작화 (변경 없음) ═══ -->
      ...
      <img class="bcf-1-rotate-1 bcf1_60" src="<%= ctx %>/img/bcf1/bcf-1-rotate-10.png" />
      <img class="bcf-1-rotate-2 bcf1_60" src="<%= ctx %>/img/bcf1/bcf-1-rotate-20.png" />
      ...
      <div class="bcf-1-dt-2 bcf1_3 bcf1_40004"></div>
      <div class="bcf-1-dt-1 bcf1_51 bcf1_40002"></div>
      <div class="bcf-1-ro-on bcf2_52"></div>
      <!-- <div class="bcf-1-ro-off"></div> -->
      <div class="bcf-1-dt-3 bcf1_50 bcf1_40060"></div>
...
(function() {
  ...
  document.querySelectorAll('[class]').forEach(function(el) {
    el.className.split(/\s+/).forEach(function(cls) {
      if (/^bcf1_\d{1,4}$/.test(cls)) {
        if (!bitElMap[cls]) bitElMap[cls] = [];
        bitElMap[cls].push(el);
      } else if (/^bcf1_\d{5,}$/.test(cls) || /^BCF1_\d+$/.test(cls)) {
  ...
  function applyData(data) {
    ...
    Object.keys(bitElMap).forEach(function(tag) {
      ...
      bitElMap[tag].forEach(function(el) {
        if (tag === 'bcf1_59') {
          ...
          el.style.animation  = isOn ? 'bcf1-spin 3s linear infinite' : 'none';
        } else if (tag === 'bcf1_60') {
          el.style.visibility = isOn ? 'visible' : 'hidden';
          el.style.animation  = isOn
            ? (el.classList.contains('bcf-1-air-cycle') ? 'bcf-air-blink 2s ease-in-out infinite' : 'bcf1-spin 3s linear infinite')
            : 'none';
        } else {
          el.style.visibility = isOn ? 'visible' : 'hidden';
        }
    ...
    /* 사이클 이미지: bcf1_39 or bcf1_40 중 하나라도 1이면 서서히 나타남 */
    var cycleOn = (data['bcf1_39'] === 1 || data['bcf1_40'] === 1);
    ['bcf1_39', 'bcf1_40'].forEach(function(tag) {
